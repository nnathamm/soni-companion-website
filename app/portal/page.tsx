...
export default async function PortalPage({ searchParams }: { searchParams: Promise<{ error?: string; notice?: string }> }) {
  ...
  return <section className="portal-shell shell shell--wide">
    ...
    {profiles.length ? <div className="portal-profile-grid">{profiles.map((profile) => <Link className="portal-profile-card" href={`/portal/profile/${profile.id}`} key={profile.id}>
      <div className="portal-profile-card__top"><span className="portal-status">{profile.status}</span><span>{profile.relationship}</span></div>
      <h2>{profile.profileLabel}</h2><p>{profile.preferredName ? `Preferred name: ${profile.preferredName}` : "No personal display name stored."}</p>
      <span className="product-link">Open profile <span aria-hidden="true">›</span></span>
    </Link>)}</div> : <div className="portal-empty"><p className="eyebrow">Nothing shared yet</p><h2>No profiles are connected to this account.</h2><p>{user.role === "admin" ? "Create the first privacy-first profile from administration." : "An administrator can connect you to a senior-approved profile."}</p>{user.role === "admin" && <Link className="button button--dark" href="/portal/admin">Open administration</Link>}</div>}
  </section>;
}
